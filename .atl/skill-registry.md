...
| `supabase-async-pattern` | Trigger: Supabase query, hook, loading state, useEffect fetch, async data, useState loading. Enforce try/catch/finally on every Supabase call so loading states never get stuck. | user | `C:\Users\USER\.claude\skills\supabase-async-pattern\SKILL.md` |
